...
public final class RedisRoundStore {
    ...
    public RoundResult claim(BigDecimal bs, int bl) {
        ...
        try (Connection redis = Connection.connect(config)) {
            int ratio = switch (outcome) {
                ...
                case SPECIAL -> requireNonEmpty(redis, true, allRatios(redis, true));
            };
    ...
    private List<Integer> positiveRatios(Connection redis, boolean special) throws IOException {
        ...
        for (int ratio : allRatios(redis, special)) if (ratio > 0) result.add(ratio);
        return result;
    }

    private List<Integer> allRatios(Connection redis, boolean special) throws IOException {
        ...
        List<Integer> result = new ArrayList<>();
        if (value instanceof List<?> list) for (Object item : list) result.add(Integer.parseInt(item.toString()));
        return result;
    }
