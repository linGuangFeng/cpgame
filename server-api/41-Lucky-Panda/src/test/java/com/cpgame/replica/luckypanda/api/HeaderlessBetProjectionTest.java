...
class HeaderlessBetProjectionTest {
    @Test void requestBetControlsEachPayoutForNewAndLegacyMembers() {
        ...
        for (var kind : RoundClass.values()) {
            ...
            var legacy = codec.decode(codec.encodeFull(source));
            for (String bs : new String[]{"0.02", "0.1", "1"}) {
                for (int bl : new int[]{1, 3, 10}) {
                    ...
                    assertEquals(codec.encodeFull(source), codec.encodeFull(legacy));
