...
public final class LoaderLimits {
    ...
    public static void checkKeys(Properties p) {
        java.util.Set<String> allowed=new java.util.HashSet<String>(java.util.Arrays.asList("generation.attempt-limit","generation.batch-size","generation.count","generation.feature-steps","generation.mary-max-win-multiplier","generation.mary-min-win-multiplier","generation.normal-max-win-multiplier","generation.normal-min-win-multiplier","generation.special-max-members-per-multiplier","generation.special-max-win-multiplier","generation.special-min-win-multiplier","mode.weight.LUCKY_FEATURE","mode.weight.LUCKY_WHEEL","mode.weight.ORDINARY_LOSS","mode.weight.ORDINARY_WIN","model.column-backoff-probability","range.normal-max","range.normal-min","range.special-max","range.special-min","redis.database","redis.db","redis.game-id","redis.host","redis.password","redis.port","retention.normal-per-multiplier","retention.special-per-multiplier"));
        allowed.addAll(java.util.List.of("redis.username", "redis.ssl", "redis.connect-timeout-ms", "redis.socket-timeout-ms"));
