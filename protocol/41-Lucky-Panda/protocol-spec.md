...
- member：无头紧凑 ASCII 完整局（兼容旧 `lp1`），禁止整份 JSON；只允许已缓存的独立无奖标记按规则物化
...
解析器继续接受旧 `lp1` 完整格式及旧格式中的 `P=#N` / `F=#N`。`encodeFull` 仅用于旧格式兼容校验，Redis Loader 默认写新格式。先更新消费端，再运行新 Loader；现有 Redis member 不会由打包自动改写。
