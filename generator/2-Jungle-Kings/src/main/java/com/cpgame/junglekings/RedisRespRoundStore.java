...
public final class RedisRespRoundStore implements RedisRoundStore {
    ...
    @Override
    public synchronized void writeMember(int betType, int ratio, byte[] member, int maximumMembers)
            throws IOException {
        ...
        String index = RedisKeys.index(betType);
        String list = RedisKeys.list(betType, ratio);
    ...
    @Override
    public synchronized List<String> ratios(int betType) throws IOException {
        Object response = command("ZRANGE", RedisKeys.index(betType), "0", "-1");
    ...
    @Override
    public synchronized long listLength(int betType, int ratio) throws IOException {
        Object response = command("LLEN", RedisKeys.list(betType, ratio));
        if (response instanceof Long value) return value;
        throw new IOException("Redis LLEN returned an unexpected type");
    }

    @Override
    public synchronized Optional<byte[]> readMember(int betType, int ratio, int offset) throws IOException {
        Object response = command("LINDEX", RedisKeys.list(betType, ratio), Integer.toString(offset));
    ...
    private void expectQueued(Object response) throws IOException {
        if (!(response instanceof String text) || !"QUEUED".equals(text)) {
            throw new IOException("Redis transaction command was not queued");
        }
    }
