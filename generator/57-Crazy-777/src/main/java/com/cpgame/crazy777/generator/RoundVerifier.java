...
public final class RoundVerifier {
    public ResultAnalysis verify(RoundResult round) {
        ...
        if (inferred.mode() != round.mode()) throw new IllegalStateException("wmkl/分类不一致");
        if (inferred.totalWin().compareTo(round.totalWin()) != 0) throw new IllegalStateException("完整局总派奖 wa 不一致");
    ...
    public void verifyLimits(RoundResult round, BigDecimal maxTotalWinMultiplier, int maxSteps) {
        ...
        if (analysis.totalMultiplier().compareTo(maxTotalWinMultiplier) > 0) {
