...
public final class RedisRespRoundStore implements RedisRoundStore {
    ...
    @Override
    public synchronized Optional<byte[]> popMember(boolean special, int ratio) throws IOException {
        Object response = command("LPOP", RedisKeys.list(special, ratio));
        if (response == null) return Optional.empty();
        if (response instanceof byte[] bytes) return Optional.of(bytes);
        throw new IOException("Redis LPOP returned an unexpected type");
    }
