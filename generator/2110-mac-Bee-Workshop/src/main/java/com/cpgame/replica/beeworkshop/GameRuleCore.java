...
/** Sole production rule authority for Bee Workshop (raw gid 2110). */
public final class GameRuleCore {
    ...
    public static final String RULES_VERSION = "bee-workshop-2110-mac-v1";
    ...
    public List<Win> evaluate(int[] board) {
        ...
        for (int line = 0; line < VISUAL_LINES.length; line++) {
            ...
            for (int reel = 0; reel < REELS; reel++) {
                ...
                if (symbol == WILD || (target != 0 && symbol == target)) {
                    count++;
    ...
    public int scatterCount(int[] board) {
        requireBoard(board);
        int count = 0;
        for (int symbol : board) if (symbol == SCATTER) count++;
        return count;
    }

    /** Original help Game2110_15; five-Scatter generation is sample-gated to observed four. */
    ...
    public void validate(CompleteRound round) {
        ...
        int expected = switch (round.kind()) {
            case ORDINARY_LOSS, ORDINARY_WIN, MYSTERY_BOX -> 1;
            case FREE_STICKY_SYMBOLS -> awardedFreeSpins(scatterCount(round.steps().get(0).board())) + 1;
        };
        ...
        long units = 0;
        LinkedHashSet<Integer> previous = new LinkedHashSet<>();
        for (int index = 0; index < round.steps().size(); index++) {
            Step step = round.steps().get(index);
            requireBoard(step.board());
            units += payoutUnits(step.board());
            String entry = round.kind() == RoundKind.FREE_STICKY_SYMBOLS
                    ? index == 0 ? "FREE_TRIGGER" : index == 1 ? "FREE_INITIAL" : index == round.steps().size() - 1 ? "FREE_TERMINAL" : "FREE_CONTINUATION"
                    : round.kind().name();
            ...
            if ((round.kind() == RoundKind.ORDINARY_LOSS || round.kind() == RoundKind.ORDINARY_WIN) && !step.specialPositions().isEmpty())
                throw new IllegalArgumentException("ordinary feature positions");
            if (round.kind() == RoundKind.MYSTERY_BOX) {
                if (step.specialPositions().isEmpty()) throw new IllegalArgumentException("mystery positions");
                int symbol = step.board()[step.specialPositions().get(0)];
                for (int p : step.specialPositions()) if (step.board()[p] != symbol) throw new IllegalArgumentException("mystery reveal");
            }
            if (round.kind() == RoundKind.FREE_STICKY_SYMBOLS) {
                ...
                if (index > 0) {
                    int previousMask = index == 1 ? 1 << 7 : 0;
                    for (int p : previous) previousMask |= 1 << p;
                    validateStickyTransition(previousMask, step, index == 1);
                    previous = new LinkedHashSet<>(step.specialPositions());
                }
            }
        }
    ...
    public void validateDealtStep(String entry, Step step) {
        ...
        for (int reel = 0; reel < REELS; reel++) {
            ...
            for (int row = 0; row < ROWS; row++) {
                int symbol = board[reel * ROWS + row];
                if (symbol == WILD) reelWild++;
                if (symbol == SCATTER) reelScatter++;
            }
    ...
    private void requireBoard(int[] board) {
        if (board == null || board.length != CELLS) throw new IllegalArgumentException("board must be 5x3");
        for (int symbol : board) if (symbol < 1 || symbol > 9) throw new IllegalArgumentException("symbol");
    }

    public int[] linePositions(int oneBasedLine) {
        if (oneBasedLine < 1 || oneBasedLine > PAYLINES) throw new IllegalArgumentException("line");
        int[] out = new int[REELS];
        for (int reel = 0; reel < REELS; reel++) out[reel] = reel * ROWS + (ROWS - 1 - VISUAL_LINES[oneBasedLine - 1][reel]);
        return out;
    }

    public int pay(int symbol, int count) { return PAY[symbol][count - 3]; }

    public static boolean special(RoundKind kind) {
        return kind == RoundKind.MYSTERY_BOX || kind == RoundKind.FREE_STICKY_SYMBOLS;
    }

    public enum RoundKind { ORDINARY_LOSS, ORDINARY_WIN, MYSTERY_BOX, FREE_STICKY_SYMBOLS }

    public record Win(int line, int num, int odd, int wp, List<Integer> positions) {
        public Win { positions = List.copyOf(positions); }
    }

    public record Step(int[] board, List<Integer> specialPositions) {
        public Step { board = board.clone(); specialPositions = List.copyOf(specialPositions); }
        @Override public int[] board() { return board.clone(); }
    }

    public record CompleteRound(RoundKind kind, List<Step> steps) {
        public CompleteRound { steps = List.copyOf(steps); }
    }
}
