...
/** Minimal RESP2 client. No Jedis in the delivery JAR. */
public final class RedisIo implements AutoCloseable {
    private final Socket socket;
    private final InputStream input;
    private final OutputStream output;

    private RedisIo(Socket socket) throws IOException {
        this.socket = socket;
        this.input = new BufferedInputStream(socket.getInputStream());
        this.output = new BufferedOutputStream(socket.getOutputStream());
    }

    public static RedisIo connect(String host, int port, String username, String password, int database,
        ...
        RedisIo connection = new RedisIo(socket);
        try {
