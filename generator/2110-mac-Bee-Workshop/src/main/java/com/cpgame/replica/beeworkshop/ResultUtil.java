...
/** Independent payout oracle. Does not call GameRuleCore.evaluate/payoutUnits. */
public final class ResultUtil {
        ...
        for (int line = 1; line <= GameRuleCore.PAYLINES; line++) {
            ...
            for (int candidate = 1; candidate <= 7; candidate++) {
                ...
                for (int position : positions) {
                    ...
                    containsCandidate |= symbol == candidate;
                    count++;
                }
    ...
    public long totalUnits(GameRuleCore.CompleteRound round) {
        return round.steps().stream().mapToLong(s -> independentPayoutUnits(s.board())).sum();
    }
