...
class RedisPackCliTest {
    ...
    @Test void generatesRandomCompleteRoundsWithOverriddenRedisGameId() throws Exception {
        ...
        String[] base = {"--normal-count", "2", "--special-count", "1",
                "--redis-game-id", "8102010", "--max-consecutive-wins", "10",
        ...
        assertEquals(2010, manifest.path("sourceGameId").asLong());
        assertEquals(8102010, manifest.path("redisGameId").asLong());
        assertEquals(0, manifest.path("ordinaryLossMembers").asInt(-1));
        ...
        assertTrue(resp.contains("PerKeyList_008102010"));
        assertTrue(resp.contains("MaryKeyList_008102010"));
        ...
        for (String line : Files.readAllLines(one.resolve("result-pack.jsonl"))) {
            ...
            for (String spin : payload.split("\\|")) assertEquals(0, spin.length() % 34);
