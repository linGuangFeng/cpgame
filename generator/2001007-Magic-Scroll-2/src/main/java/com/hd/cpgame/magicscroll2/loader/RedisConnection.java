...
/** 最小 RESP2 客户端，最终带依赖 JAR 不要求 redis-cli。 */
final class RedisConnection implements AutoCloseable {
    ...
    static RedisConnection connect(GeneratorConfig config) throws IOException {
        ...
        try {
            ...
            System.out.println("REDIS_CONNECTED host=" + config.host + " port=" + config.port
                    + " database=" + config.database + " ssl=" + config.ssl);
            return connection;
        } catch (Exception ex) {
            connection.close();
            if (ex instanceof IOException) throw (IOException) ex;
            throw new IOException("Redis 初始化失败", ex);
        }
    }
    ...
    void transaction(List<String[]> body) throws IOException {
        ...
        if (!"OK".equals(replies.get(0))) throw new IOException("Redis MULTI 未返回 OK");
        for (int i = 1; i <= body.size(); i++) {
            if (!"QUEUED".equals(replies.get(i))) throw new IOException("Redis 事务命令未进入队列");
        }
        Object exec = replies.get(replies.size() - 1);
        if (!(exec instanceof List) || ((List<?>) exec).size() != body.size()) {
            throw new IOException("Redis EXEC 响应数量不匹配");
        }
    }
